...
db.delete(`bayanrol_${message.guild.id}`)
  ...
  let bayanrol = message.mentions.roles.first() || message.guild.roles.cache.get(args.join(' '))
  if (!bayanrol) return message.channel.send(new Discord.MessageEmbed()
                                              .setColor('BLUE')
                                              .setDescription(`Bayan Rolünü Belirlemelisiniz.`))
  db.set(`bayanrol_${message.guild.id}`, bayanrol.id)
  if (!message.guild.roles.cache.get(bayanrol.id)) return message.channel.send(new Discord.MessageEmbed()
                                              .setColor('BLUE')
